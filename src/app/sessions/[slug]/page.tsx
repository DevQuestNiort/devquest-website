...
const Session = async ({ params: { slug, title } }: SessionProps) => {
  ...
  const myFullSession = { ...session, slot: slots.find(s => s.key === session.slot) } as FullSession;
  ...
  return <div className={styles.container}>
    ...
    <div className={styles.informations}>
      ...
      <div className={styles.tags}>{session?.tags.map((tag: TagsModel) => (
        <Chip
      ...
      <div className={styles.speakers}>
        {myFullSession.speakers.map(speaker => <div key={speaker.id} className={styles.speaker}>

          <Avatar classes={{
            main: styles.avatar,
            icon: styles.icon
          }} name={speaker.name} img={speaker.photo} github={speaker.social?.github} linkedin={speaker.social?.linkedin} />

          <p className={styles.bio}>{speaker.bio ?? `Rejoins-moi à ${myFullSession.slot.start}`}</p>
        </div>)}
